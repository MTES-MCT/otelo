...
export const ModifyAggregatedParcsComparisonChart = ({ targetYear }: { targetYear: number | null }) => {
  ...
  const aggregateRates = () => {
    ...
    epciIds.forEach((epciId) => {
      ...
      if (originalRates && currentRates) {
        ...
        totalOriginalLongTerm += originalRates.longTermVacancyRate
        totalComputedLongTerm += currentRates.longTermVacancyRate
      }
    })
  ...
  return (
    <div className="fr-border-top fr-p-3v fr-flex fr-direction-column fr-justify-content-space-between">
      ...
      {isShown && (
        <ChartDownloadWrapper fileName="comparaison-parc-agregee">
